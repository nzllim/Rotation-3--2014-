...
# Wild Type (K); Time = 0 and 30
small.meta <- subset(frag.matrix, frag.matrix$Type %in% 'K' & frag.matrix$Timepoint %in% c('0', '30'))
small.meta$Timepoint <- factor(small.meta$Timepoint)
...
sm.design <- model.matrix(~Timepoint, small.meta)
...
# **Current Null Hypothesis: TypeS (i.e. WT at time = 0 against 30)
dge.fit <- glmFit(dge.tag.list, sm.design)
dge.lrt <- glmLRT(dge.fit, coef = 'Timepoint30')
...
write.table(export.dge.cpm, file = 'EdgeR WT Normalized Counts.tsv', sep = '\t', row.names = FALSE, col.names = TRUE)
...
write.table(export.final.top, file = 'EdgeR WT Top Hits.tsv', sep = '\t', row.names = FALSE, col.names = TRUE)