...
# Sxy (S); Time = 0 and 30
small.meta <- subset(frag.matrix, frag.matrix$Type %in% 'S' & frag.matrix$Timepoint %in% c('0', '30'))
...
# **Current Null Hypothesis: TypeS (i.e. Sxy at time = 0 against 30)
...
write.table(export.dge.cpm, file = 'EdgeR Sxy Normalized Counts.tsv', sep = '\t', row.names = FALSE, col.names = TRUE)
...
write.table(export.final.top, file = 'EdgeR Sxy Top Hits.tsv', sep = '\t', row.names = FALSE, col.names = TRUE)