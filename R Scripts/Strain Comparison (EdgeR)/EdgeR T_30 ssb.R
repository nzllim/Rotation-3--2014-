...
small.meta <- subset(frag.matrix, frag.matrix$Type %in% c('K', 'S') & frag.matrix$Timepoint %in% '30')
...
ssb.qvalue <- dge.top$table['HI_0250', ]$FDR

final.top.table <- dge.top$table[dge.top$table$FDR <= ssb.qvalue, ]
...
write.table(export.dge.cpm, file = 'EdgeR T30 ssb Normalized Counts.tsv', sep = '\t', row.names = FALSE, col.names = TRUE)
...
write.table(export.final.top, file = 'EdgeR T30 ssb Top Hits.tsv', sep = '\t', row.names = FALSE, col.names = TRUE)