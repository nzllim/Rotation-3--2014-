...
# Wild Type and Sxy (K and S); Time = 10
small.meta <- subset(frag.matrix, frag.matrix$Type %in% c('K', 'S') & frag.matrix$Timepoint %in% '10')
...
# **Current Null Hypothesis: TypeS (i.e. WT against sxy at time = 10)
...
write.table(export.dge.cpm, file = 'EdgeR T10 Normalized Counts.tsv', sep = '\t', row.names = FALSE, col.names = TRUE)
...
write.table(export.final.top, file = 'EdgeR T10 Top Hits.tsv', sep = '\t', row.names = FALSE, col.names = TRUE)