...
# Wild Type and Sxy (K and S); Time = 100
small.meta <- subset(frag.matrix, frag.matrix$Type %in% c('K', 'S') & frag.matrix$Timepoint %in% '100')
...
# **Current Null Hypothesis: TypeS (i.e. WT against sxy at time = 100)
...
write.table(export.dge.cpm, file = 'EdgeR T100 Normalized Counts.tsv', sep = '\t', row.names = FALSE, col.names = TRUE)
...
write.table(export.final.top, file = 'EdgeR T100 Top Hits.tsv', sep = '\t', row.names = FALSE, col.names = TRUE)